# Prajwal Suryawanshi

Data Science | Backend Developer  
I build technology that transforms complexity into simplicity at scale.

## About

- Currently learning Data Science from IIT Madras
- Based in Nanded, Maharashtra
- Fun fact: I am also a good chess player

## Connect

- Email: [prajwal221104@gmail.com](mailto:prajwal221104@gmail.com)
- LinkedIn: [linkedin.com/in/prajwalsuryawanshi](https://linkedin.com/in/prajwalsuryawanshi)
- GitHub: [github.com/prajwalsuryawanshi](https://github.com/prajwalsuryawanshi)
- Kaggle: [kaggle.com/prajwalhere](https://kaggle.com/prajwalhere)
- Instagram: [instagram.com/prjwl._](https://instagram.com/prajwalsuryawanshii)
- HackerRank: [hackerrank.com/prajwal221104](https://www.hackerrank.com/prajwal221104)

## Focus

- Backend engineering: Django, Flask, FastAPI, REST, Microservices
- Data & ML: SQL, PostgreSQL, MongoDB, Pandas, TensorFlow
- Cloud & DevOps: AWS, GCP, Docker, Kubernetes, CI/CD

## Highlights

- Designed real-time queue management supporting 300+ concurrent users
- Implemented secure authentication systems (OAuth 2.0, RBAC)
- Built scalable systems, CI/CD pipelines, and API security in microservices

---
If you want to collaborate or discuss ideas, feel free to reach out.
